...
GENEL transient run-out cikarici (cell-center). Mesh boyutu blockMeshDict'ten OTOMATIK okunur.
  ...
  r_over_r0 = r/R0, h_over_h0 = h/H0   (R0=H0=1.0)
Hucre siralamasi (pseudo-2D 1xNYxNZ blok): index i = iy + iz*NY.
Cikti: <case>/r_h_transient.csv (t,r_over_r0,h_over_h0)
...
def main():
    ...
    for t, d in times:
        ...
        rows.append((t, r / R0, h / H0))
    out = os.path.join(case, 'r_h_transient.csv')
    with open(out, 'w') as f:
        f.write('t,r_over_r0,h_over_h0\n')
        for t, r, h in rows:
            f.write('%.4f,%.4f,%.4f\n' % (t, r, h))
